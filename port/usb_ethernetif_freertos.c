...
static void USB_HostCdcEcmControlCallback(void *param, uint8_t *data, uint32_t dataLength, usb_status_t status)
{
    ...
    switch (ecmInstance->runPrevState)
    {
        ...
        case USB_HostCdcEcmRunSetEthernetMulticastFilters:
            xEventGroupSetBits(ecmInstance->netifUsbRequestEvent, CDC_ECM_REQUEST_SET_ETH_MC_FL);
            if (netif_is_link_up(netif))
            {
                ecmInstance->runCurState = USB_HostCdcEcmRunDataRecv;
            }
            else
            {
                ecmInstance->runCurState = USB_HostCdcEcmRunNetworkConnected;
            }
            ecmInstance->runWaitState = USB_HostCdcEcmRunIdle;
            break;

        case USB_HostCdcEcmRunSetEthernetPowerManagementPatternFilter:
            xEventGroupSetBits(ecmInstance->netifUsbRequestEvent, CDC_ECM_REQUEST_SET_ETH_PWR_MGMT_PAT_FL);
            if (netif_is_link_up(netif))
            {
                ecmInstance->runCurState = USB_HostCdcEcmRunDataRecv;
            }
        ...
        case USB_HostCdcEcmRunGetEthernetPowerManagementPatternFilter:
            xEventGroupSetBits(ecmInstance->netifUsbRequestEvent, CDC_ECM_REQUEST_GET_ETH_PWR_MGMT_PAT_FL);
            if (netif_is_link_up(netif))
            {
                ecmInstance->runCurState = USB_HostCdcEcmRunDataRecv;
            }
        ...
        case USB_HostCdcEcmRunSetEthernetPacketFilter:
            xEventGroupSetBits(ecmInstance->netifUsbRequestEvent, CDC_ECM_REQUEST_SET_ETH_PKT_FL);
            if (netif_is_link_up(netif))
            {
                ecmInstance->runCurState = USB_HostCdcEcmRunDataRecv;
            }
        ...
        case USB_HostCdcEcmRunGetEthernetStatistic:
            xEventGroupSetBits(ecmInstance->netifUsbRequestEvent, CDC_ECM_REQUEST_GET_ETH_STATS);
            if (netif_is_link_up(netif))
            {
                ecmInstance->runCurState = USB_HostCdcEcmRunDataRecv;
            }
...
static void USB_HostCdcEcmDataInCallback(void *param, uint8_t *data, uint32_t dataLength, usb_status_t status)
{
    ...
    taskENTER_CRITICAL();
    if (ecmInstance->devCurState == USB_DeviceStateAttached)
    {
        if (status != kStatus_USB_Success)
        {
            ecmInstance->runCurState = USB_HostCdcEcmRunNetworkConnected;
    ...
    else
    {
        ...
        switch (notify_bNotificationCode)
        {
            case USB_HOST_CDC_NOTIFICATION_NETWORK_CONNECTION:
                if (ecmInstance->runPrevState == USB_HostCdcEcmRunNetworkConnected)
                {
                    ecmInstance->runCurState = USB_HostCdcEcmRunNetworkSpeedChange;
            ...
            case USB_HOST_CDC_NOTIFICATION_CONNECTION_SPEED_CHANGE:
                if (ecmInstance->runPrevState == USB_HostCdcEcmRunNetworkSpeedChange)
                {
                    ecmInstance->runCurState = USB_HostCdcEcmRunNetworkConnected;
                    ...
                    if (ecmInstance->deviceNetworkConnection && ecmInstance->deviceNetworkDownLinkSpeed &&
                    ...
                    else if (!ecmInstance->deviceNetworkConnection && !ecmInstance->deviceNetworkDownLinkSpeed &&
                             !ecmInstance->deviceNetworkUpLinkSpeed)
                    {
                        if (netif_is_link_up(netif))
                        {
...
void USB_HostCdcEcmTask(void *param)
{
    ...
    switch (ecmInstance->devWaitState)
    {
        ...
        case USB_DeviceStateDetached:
            ...
            ecmInstance->deviceNetworkConnection    = 0U;
            ecmInstance->deviceNetworkDownLinkSpeed = 0U;
            ecmInstance->deviceNetworkUpLinkSpeed   = 0U;
    ...
    switch (ecmInstance->runCurState)
    {
        ...
        case USB_HostCdcEcmRunNetworkConnected:
            ecmInstance->runPrevState = ecmInstance->runCurState;
            ecmInstance->runCurState  = USB_HostCdcEcmRunIdle;
            if (USB_HostCdcInterruptRecv(ecmInstance->classHandle, ecmInstance->notifyBuffer,
                                         CDC_ECM_NOTIFY_NETWORK_CONNECTION_LEN, USB_HostCdcEcmInterruptCallback,
                                         ecmInstance) != kStatus_USB_Success)
            {
                usb_echo("Error in USB_HostCdcInterruptRecv NetworkConnection.\r\n");
            }
            break;

        case USB_HostCdcEcmRunNetworkDisconnect:
            ecmInstance->runPrevState = ecmInstance->runCurState;
            ecmInstance->runCurState  = USB_HostCdcEcmRunNetworkConnected;
            ecmInstance->runWaitState = USB_HostCdcEcmRunIdle;
            break;

        case USB_HostCdcEcmRunNetworkSpeedChange:
            ecmInstance->runPrevState = ecmInstance->runCurState;
            ecmInstance->runCurState  = USB_HostCdcEcmRunIdle;
            if (USB_HostCdcInterruptRecv(ecmInstance->classHandle, ecmInstance->notifyBuffer,
                                         CDC_ECM_NOTIFY_NETWORK_SPEED_CHANGE_LEN, USB_HostCdcEcmInterruptCallback,
                                         ecmInstance) != kStatus_USB_Success)
            {
                usb_echo("Error in USB_HostCdcInterruptRecv ConnectionSpeedChange.\r\n");
            }
            break;

        case USB_HostCdcEcmRunDataRecv:
            ecmInstance->runPrevState = ecmInstance->runCurState;
            ecmInstance->runCurState  = USB_HostCdcEcmRunIdle;
            USB_HostCdcEcmDataRecv(ecmInstance->classHandle, ecmInstance->dataRecvBuffer, CDC_ECM_DATA_BUFFER_LEN,
                                   USB_HostCdcEcmDataInCallback, ecmInstance);
            break;
...
static void USB_HostApplicationInit(uint8_t controllerId, struct netif *netif)
{
    ...
    g_HostCdcEcmInstance.netifUsbRequestEvent       = xEventGroupCreate();
...
#if defined(USB_HOST_CONFIG_CDC_ECM) && USB_HOST_CONFIG_CDC_ECM
    if (g_HostCdcEcmInstance.runPrevState == USB_HostCdcEcmRunDataRecv)
    {
        if (g_HostCdcEcmInstance.deviceMCFilters >= usedFilters && g_HostCdcEcmInstance.deviceMCFilters)
        {
            if (!(g_HostCdcEcmInstance.devicePktFilerBitmap & CDC_ECM_ETH_PACKET_FILTER_MULTICAST))
            {
                ...
                g_requestParam->SetEthernetPacketFilter = g_HostCdcEcmInstance.devicePktFilerBitmap;
                g_HostCdcEcmInstance.runPrevState       = USB_HostCdcEcmRunSetEthernetPacketFilter;
                if (USB_HostCdcEcmSetEthernetPacketFilter(
                        g_HostCdcEcmInstance.classHandle, g_requestParam->SetEthernetPacketFilter,
                        USB_HostCdcEcmControlCallback, &g_HostCdcEcmInstance) != kStatus_USB_Success)
                {
                    usb_echo("Send SetEthernetPacketFilter request error.\r\n");
                }
                xEventGroupWaitBits(g_HostCdcEcmInstance.netifUsbRequestEvent, CDC_ECM_REQUEST_SET_ETH_PKT_FL, pdTRUE,
                                    pdFALSE, portMAX_DELAY);
            }

            g_requestParam->SetEthernetMulticastFilters.filterNumSum         = filterLen;
            g_requestParam->SetEthernetMulticastFilters.multicastAddressList = (uint8_t(*)[6])filter;
            g_HostCdcEcmInstance.runPrevState = USB_HostCdcEcmRunSetEthernetMulticastFilters;
            if (USB_HostCdcEcmSetEthernetMulticastFilters(
                    g_HostCdcEcmInstance.classHandle, g_requestParam->SetEthernetMulticastFilters.filterNumSum,
                    g_requestParam->SetEthernetMulticastFilters.multicastAddressList, USB_HostCdcEcmControlCallback,
                    &g_HostCdcEcmInstance) != kStatus_USB_Success)
            {
                usb_echo("Send SetEthernetMulticastFilters request error.\r\n");
            }
            xEventGroupWaitBits(g_HostCdcEcmInstance.netifUsbRequestEvent, CDC_ECM_REQUEST_SET_ETH_MC_FL, pdTRUE,
                                pdFALSE, portMAX_DELAY);
        }
        else if (usedFilters)
        {
